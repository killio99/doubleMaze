...
class Player1 {
    ...
    update(timeStep, state, keys) {
        ...
        let altMovedX = state.players[1].pos.plus(new Vec(xSpeed * timeStep, 0));
        //console.log(state.level.touches(state.players[1].pos, state.players[1].size, "wall"));
        if (!state.level.touches(movedX, this.size, "wall") &&
            !state.level.touches(altMovedX, state.players[1].size, "wall") ) {
            pos = movedX;
        }

        let movedY = pos.plus(new Vec(0, ySpeed * timeStep));
        let altMovedY = state.players[1].pos.plus(new Vec(0, ySpeed * timeStep));
        if (!state.level.touches(movedY, this.size, "wall") &&
        !state.level.touches(altMovedY, state.players[1].size, "wall")) {
            pos = movedY;
        }
        
        return new Player1(pos, new Vec(xSpeed, ySpeed), this.completed);
      }
}
...
class Player2 {
  ...
  update(timeStep, state, keys) {
    ...
    let pos = this.pos;
    let movedX = pos.plus(new Vec(xSpeed * timeStep, 0));
    let altMovedX = state.players[0].pos.plus(new Vec(xSpeed * timeStep, 0));
    
    if (!state.level.touches(movedX, this.size, "wall") && 
    !state.level.touches(altMovedX, state.players[0].size, "wall") ) {
      pos = movedX;
    }

    let movedY = pos.plus(new Vec(0, ySpeed * timeStep));
    let altMovedY = state.players[0].pos.plus(new Vec(0, ySpeed * timeStep));

    if (!state.level.touches(movedY, this.size, "wall") &&
      !state.level.touches(altMovedY, state.players[0].size, "wall")) {
        pos = movedY;
    }
    console.log(this.completed);
    return new Player2(pos, new Vec(xSpeed, ySpeed), this.completed);
    }
}
